...
//    }
    ...
    @RolesAllowed({"user", "admin"})
    public String getFollowedPlaylists(@PathParam("username") String username) throws API_Exception {
        PlaylistFacade pf = PlaylistFacade.getPlaylistFacade(EMF);
        List<MyPlaylistDTO> response;
        try {
            response = pf.getFollowedPlaylists(username);
        } catch (Exception e) {
            throw new API_Exception("Failed to get followed playlists", 400, e);
        }
